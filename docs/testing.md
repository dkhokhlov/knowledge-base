...
| `tests/test_09_gdrive_index.sh` | gdrive index via kb-gateway: `POST /index` (admin) reconciles `./gdrive` into the KB, polls `GET /status` until the drain is terminal (`pending+processing=0` AND `completed+failed>=source`), fails on any "Failed to link" error (a double-link race — the background task must be the sole linker), surfaces genuine failures as a notice, and runs a deterministic semantic search (first completed file's stem → require ≥1 hit). SKIPs when `GDRIVE_KB_ID` is unset or `./gdrive` has no allowlisted files | `OPENWEBUI_ADMIN_API_KEY`, `OPENWEBUI_USER_API_KEY` |
...
- `test_09` SKIPs (passes with a notice) when `GDRIVE_KB_ID` is unset or `./gdrive` has no allowlisted files; `test_10` SKIPs when the OCR sidecar is not provisioned (`MARKITDOWN_OCR_PROVISIONED!=1`) — so `make test` runs clean in a bare environment. test_09 uses kb-gateway (`POST /index` + `GET /status`), a core service (not a sidecar). The full clean-state run that provisions the OCR sidecar + the gdrive KB is `make test-e2e` (destructive: wipes `./data` + re-provisions; runs `make gdrive-sync` then `make test`).
