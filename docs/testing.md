...
- `test_09` is not part of `make test` (the Makefile skips it with a notice); it runs the full real-gdrive drain under `make test-e2e`, which invokes it by path after the fresh `gdrive-sync`. It still SKIPs (passes with a notice) when run standalone with `GDRIVE_KB_ID` unset or `./gdrive` empty. `test_10` SKIPs when the OCR sidecar is not provisioned (`MARKITDOWN_OCR_PROVISIONED!=1`) — so `make test` runs clean in a bare environment. `test_11` uses kb-gateway (`POST /index` + `GET /status` with the `path` param), a core service (not a sidecar), against committed `gdrive/.tests/` fixtures (a dot-dir the full gdrive walk skips, so the real gdrive KB is never contaminated). The full clean-state run that provisions the OCR sidecar + the gdrive KB is `make test-e2e` (destructive: wipes `./data` + re-provisions; runs `make gdrive-sync`, then `make test`, then `test_09` by path).
