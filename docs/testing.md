...
| `tests/test_09_gdrive_index.sh` | gdrive index via kb-gateway: `POST /index` (admin) reconciles `./gdrive` into the KB, then polls `GET /status` (read) until `pending=0` and asserts `indexed >= source - errors`; best-effort semantic search (a hit is a bonus, not a hard requirement). SKIPs when `GDRIVE_KB_ID` is unset or `./gdrive` has no allowlisted files | `OPENWEBUI_ADMIN_API_KEY`, `OPENWEBUI_USER_API_KEY` |
