...
- `.env.local` — gitignored, `chmod 0600`. Holds:
  ...
  - `HOST_UID` / `HOST_GID` (generated by `make bootstrap` from the current user's `id -u`/`id -g`; used for the kb-gateway `user:` directive in `compose.yml`, not as container env).
...
| `HOST_UID` / `HOST_GID` | kb-gateway runs as this uid:gid (in `.env.local`, generated by `make bootstrap` from the current user's `id -u`/`id -g`; default 1000:1000) so the read-only `./gdrive` bind mount (0700/0600, owner-only from rclone) is readable |
...
- `.env.local` is gitignored (`chmod 0600`) and holds:
  ...
  - `OPENWEBUI_USER` / `OPENWEBUI_USER_PASSWORD` (the agent account; password generated by `make api-keys`).
  - `OCR_SERVICE_TOKEN` (generated by `make bootstrap` when `OCR_ENABLED=true`; the bearer OWUI sends to `markitdown-ocr`; absent when `OCR_ENABLED=false`).
  - `HOST_UID` / `HOST_GID` (generated by `make bootstrap` from the current user's `id -u`/`id -g`; used for the kb-gateway `user:` directive, not injected as container env).
