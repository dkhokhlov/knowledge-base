...
- `graphiti/bootstrap.py` — mounted into the `graphiti` container and run as the command. The `zepai/graphiti` REST server defaults to `OpenAIClient` (OpenAI Responses API), which [Ollama][ollama] cannot satisfy — extraction silently stores nothing. The bootstrap overrides the FastAPI `get_graphiti` dependency to inject an `OpenAIGenericClient` subclass (`_SchemaEnforcingClient`) that requests **`json_schema` structured outputs** (Ollama >= 0.32 enforces the response schema server-side; plain `json_object` lets a 14B model echo the schema, which Neo4j rejects) + `OpenAIEmbedder`([`nomic-embed-text`][nomic-embed-text], 768-dim), builds the vector index at 768 once at startup, and runs a robust `/messages` worker that logs + isolates per-episode failures. There is no env switch for the client; this injection is required. See `graphiti/bootstrap.py`.
...
| `OPENWEBUI_IMAGE_TAG` / `GRAPHITI_IMAGE_TAG` / `NEO4J_IMAGE_TAG` | pinned image tags (`GRAPHITI_IMAGE_TAG` = `zepai/graphiti` REST server; default `0.22.0`) |
...
| `MODEL_NAME` / `OPENWEBUI_MODEL` | chat + extraction LLM (keep in sync; the skill reads `OPENWEBUI_MODEL`) |
...
| `add` returns `200` but the fact never appears in `search` | extraction failed or is still running — `add` is async (202) and each episode runs several LLM calls (~20 s each) before a fact is searchable | poll `/memory/search` for the probe token for up to ~5 min; check `docker logs kb-graphiti` — the LLM must hit `OLLAMA_HOST/v1/chat/completions` (not `/v1/responses`, not `api.openai.com`); if it hits `api.openai.com` → `OPENAI_BASE_URL` is wrong; if `/v1/responses` → the bootstrap injection is missing/broken |
| `add` returns `200`, episode is stored, but no fact ever appears; `docker logs kb-graphiti` shows `Expecting value: line 1 column 1 (char 0)` | `MODEL_NAME` is a **reasoning model** (e.g. `gemma4:12b`): its thinking chain exhausts the 8192-token budget, `content` is empty (`finish_reason=length`), and `json.loads('')` fails | set `MODEL_NAME` to a **non-reasoning** model (`qwen2.5:14b`), `make restart`, re-add. Ollama `/v1/chat/completions` ignores `think=false`, so you cannot suppress reasoning that way |
| `add` returns `200` but no fact; `docker logs kb-graphiti` shows `CypherTypeError ... Property values can only be of primitive types ... Map{summary -> Map{...}}` | the `_SchemaEnforcingClient` json_schema override is not in effect (image bump / bootstrap edit), so the model runs in `json_object` mode and echoes the response schema as values; Neo4j rejects the nested MAP | confirm `graphiti/bootstrap.py` is mounted and `command: ["python","/app/bootstrap.py"]`; restart `graphiti`; the worker log should show extraction completing without the Cypher error |
