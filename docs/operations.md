...
- `.env` — gitignored, copied from `.env.template` (the tracked template). No secrets; `OLLAMA_HOST` + `KB_HOST` are commented out (deployment-specific) — set them in your shell env, or uncomment here:
  - ports, image tags, model names, Neo4j memory, tunables, `OLLAMA_HOST`, `OCR_ENABLED` (OCR config flag, default `true`; decided at provision time — `make provision` bakes `COMPOSE_PROFILES=ocr` into `.env` from `OCR_ENABLED`; disable with `make clean-all && make provision OCR_ENABLED=false`; no per-run toggle, the sidecar is governed by `.env`).
- `OLLAMA_HOST` — Ollama's native client env var: the full URL where [Graphiti][graphiti] and [Open WebUI][open-webui] reach [Ollama][ollama]. Set in shell env or `.env` (shell overrides `.env`). `localhost`/`127.0.0.1` is allowed (the shell convention for a host-local Ollama): the graphiti + markitdown-ocr containers run a small entrypoint shim (`scripts/ollama-host.sh`) that rewrites it to `host.docker.internal` before the app reads it (compose adds `extra_hosts: host-gateway`). A non-localhost URL (a remote/LAN GPU host) passes through unchanged. See [Repointing OLLAMA_HOST](#repointing-ollama_host) for the move procedure.
  - [Graphiti][graphiti] reads it fresh on every start (compose sets `OPENAI_BASE_URL=$OLLAMA_HOST/v1` for the REST server, and the graphiti container's native `MODEL_NAME` var (mapped from `.env` `GRAPHITI_MODEL`) / `EMBEDDING_MODEL_NAME` / `EMBEDDER_DIMENSIONS` from `.env`); the shim translates `localhost`→`host.docker.internal` at container start.
  - [Open WebUI][open-webui] persists the Ollama URL in its DB on **first boot** and ignores later env changes. It does **not** use the shim; `make config-rag` (part of `make provision`) writes the translated value (`host.docker.internal`) to the DB.
    - Chat URL: admin API `POST /ollama/config/update` with `{"ENABLE_OLLAMA_API":true,"OLLAMA_BASE_URLS":["http://<ollama-host>:11434"],"OLLAMA_API_CONFIGS":{}}`, or wipe `./data/openwebui` and restart. Use the **container-reachable** value (`http://host.docker.internal:11434` if Ollama is host-local), not `localhost` — OWUI chat does **not** use the entrypoint shim, so a `localhost` chat URL points at the container's own loopback (no Ollama). `make config-rag` does not touch this key either.
    - RAG embedding URL (`rag.ollama.base_url`) is a **separate** persisted key; the chat update above does **not** change it. A stale value leaves chat working but breaks file embedding (`process/status` = `failed`, RAG search returns 0 hits). `make config-rag` syncs it to the translated `OLLAMA_HOST` (`host.docker.internal`, not `localhost` — the same translation the shim applies; idempotent); `make preflight` warns if it has drifted. `make test` (test_04) catches this drift.
- `.env.local` — gitignored, `chmod 0600`. Holds:
  - `WEBUI_SECRET_KEY` (generated by `make bootstrap`).
  - `OPENWEBUI_ADMIN_API_KEY` (generated by `make api-keys`) — the admin key, a valid `KB_API_KEY` for the api-gateway (admin role + override).
  - Operator user keys (`KB_API_KEY`, non-admin, read-scoped) come from `make users-create` and live in `~/.api_keys`, not `.env.local`.
...
| `KB_DOMAIN` | `.env` (default `local.test`) | email domain for provisioned accounts: first user `admin@<domain>`; override one run with `make bootstrap KB_DOMAIN=<domain>`, or set here for a persistent custom domain; change requires `make clean-all && make bootstrap` to recompute the first-user email |
...
- `KB_API_KEY` is an Open Web UI per-account API key, carried as an **env var**. It is set on the agent host (in that host's env file or shell); on the stack host it may also live in `.env.local`. Its value is one of:
  ...
  - a non-admin user key (`KB_API_KEY`, read-scoped, from `make users-create`), or
...
4. Edit host-specific values in the restored `.env`:
   ...
   - `KB_HOST`, `KB_DOMAIN` — as needed for the destination network (`KB_HOST_PORT` is re-derived from `KB_HOST` on the next `make bootstrap`; set it explicitly only for the tunnel case).
...
is one KB, named after the subdir (`./root/gdrive/` is the gdrive KB). A KB
carries a structured source attribute as kv in its `description`:
`source=root | host=<hostname> | path=<name>` for root KBs, or
`source=projects-memory | host=.. | project=.. | repo=.. | path=..` for
projects-memory KBs. OWUI's REST API cannot write the `meta` JSONB field, so
the attribute lives in the writable `description` (prose lead kept); `kb kbs`
parses the kv. `make kb-desc-backfill` (one-time, idempotent) appends the kv to
legacy KB descriptions. The
`gdrive` KB is indexed by **api-gateway** (stateless, no sidecar). The gdrive
pipeline is split into two stages: `make kb-sync` runs rclone to sync
`./root/gdrive` from the shared drives (SYNC-ONLY; does NOT index), then
`make kb-index KB=gdrive` POSTs `/index?dir=gdrive` to api-gateway, which walks
`./root/gdrive` read-only and drives OWUI's native sync/diff protocol to reconcile
the tree into the KB. The split makes sync and index independently retryable.
Indexing is **manual/on-demand only**: no daemon, no schedule, no hooks.
`make kb-index KB=<name>` runs POST `/index` alone (no rclone); `make kb-status
KB=<name>` reads GET `/status`. `make kb-index` fails fast before dispatch if a
drain is already in flight for that KB (`pending+processing>0`; exempt
`RETRY_PENDING=1`); raw `curl POST /index` is not guarded. The drain is async
(OWUI background extract→embed→link), so after `make kb-index KB=gdrive` the
vectors are written but — on pgvector/ivfflat — not yet queryable: ivfflat
...
runs that `REINDEX` (ivfflat vector) once the drain is terminal; `make
kb-index-finalize` dispatches the drain (`make kb-index`) then waits for
`pending+processing=0` and finalizes in one command. `REINDEX` is instance-wide
...
pgvector is the only supported backend: `kb-finalize` fails loud on any other
`VECTOR_DB` (pgvector's ivfflat index needs `REINDEX` after a bulk drain; HNSW-style
indexes are incremental and do not). Named "finalize" (not "reindex") because the
fresh vectors were never queryable (a first publish, not a re-do) and to avoid
collision with the gateway `reindex_all` (`POST /index?reindex_all=1` re-PROCESSES
files — a different operation). The FTS arm uses ParadeDB `pg_search` BM25
(`idx_document_chunk_bm25`, a `USING bm25` index created by `make kb-bm25-init`),
which is incremental — queryable immediately after a bulk drain, no `REINDEX`
needed; the old GIN FTS index is gone. Only ivfflat needs the finalize `REINDEX`.
...
/ stuck-pending symptom). This orphan-delete fires on every reconcile (every
reconcile is KB-wide — `dir=<name>` selects the whole top dir) and only for a
file carrying a gateway `file_hash` (a gateway-managed file); a non-gateway
file or a no-source `completed` file (`sync/diff`'s job — it is linked) is
skipped, not deleted.
...
tree at `./root/<name>/` and run `make kb-bootstrap KB=<name>` + `make kb-index KB=<name>`.

### Populating the source

**Prerequisite — rclone `gdrive` remote (Google Drive login).** `make kb-sync`
reads the source through rclone's `gdrive` remote (default name;
...
you index; `make kb-sync` fail-fasts if no drives are visible. Re-login
...
`make kb-sync` runs `rclone copy --files-from --backup-dir` of the shared
...
hierarchy) into a dated `./.gdrive-backup/<UTC-ISO>/` dir, which sits OUTSIDE
`./root` so `/index` does not index them. That backup dir is the recovery net
for a bad/empty mount: an empty Drive enumeration empties `./root/gdrive` — but
the removed files are in `./.gdrive-backup/`, not gone. `make clean-backup`
...
KB. The `kb-sync` download stage enumerates the per-drive remote file list
...
`make kb-status KB=<name>` reads GET `/status?dir=<name>` (api-gateway). `/status` reports real
...
kb-index KB=gdrive` to re-trigger them, or `make kb-index KB=gdrive
RETRY_PENDING=1` if a file is stuck `pending`.
...
new/changed files — the upload-idempotency patch early-returns the existing
FileModel on a hash match without touching `File.meta`, so already-indexed
memory files stay `mtime: null` until they change). The KB `description` carries
the source-attribute kv: `Claude projects memory | source=projects-memory |
host=.. | project=.. | repo=.. | path=<project-dir>` (parsed by `kb kbs`).
...
no runtime toggle); compose-profile-gated (`COMPOSE_PROFILES=ocr` in `.env`,
baked by `make bootstrap` from `OCR_ENABLED` and read by `docker compose` for
every command); no fallback. When enabled it is provisioned by the standard
chain (token + `COMPOSE_PROFILES=ocr` via `bootstrap`, model via `pull-models`,
sidecar via `start`, OWUI routing via `api-keys`); no separate step. To
disable, run `make clean-all && make provision OCR_ENABLED=false`. Full
...
| RAG search returns 0 hits; file embedding `process/status = failed` | `rag.ollama.base_url` stale (OWUI persists it on first boot, ignores later `OLLAMA_HOST` changes) | `make config-rag` (re-syncs to the translated `OLLAMA_HOST`); `make preflight` warns on drift; test_04 catches it |
| RAG chat confabulates (wrong vendor, invented file names) | strict-grounding RAG template not set (model falls back to its own knowledge) | `make config-rag`; re-run after any DB reset/rebuild (`make clean-all` reverts it) |
...
| `provision` | ONE-TIME from-scratch setup: `bootstrap` → `pull-models` (blocking) → `start` → `admin-signup` → `api-keys` (auto OCR config) → `config-rag` → `kb-bm25-init` → `kb-bootstrap KB=gdrive`; leaves the stack running. Re-run after `make clean-all`; for everyday restart use `make start` |
...
| `config-rag` | set the strict-grounding RAG template + sync `rag.ollama.base_url` to the translated `OLLAMA_HOST` (run after `make api-keys`; re-run after a DB reset or an `OLLAMA_HOST` change; see [Repointing OLLAMA_HOST](#repointing-ollama_host)) |
| `config-ocr` | re-assert the OWUI external-extraction keys (engine + URL + API key); auto-set by `make api-keys` when `OCR_ENABLED=true`; re-run after a DB reset; no-ops (exit 0) when `OCR_ENABLED!=true` |
| `kb-sync` | sync external sources into `./root/` (SYNC-ONLY; does NOT index). Today this is gdrive: rclone `sync --backup-dir --delete-after` the shared drives into `./root/gdrive` (delta; deleted/overwritten files retained in `./.gdrive-backup/`); fail-fast on any transfer error; name-collision guard; concurrency lock (`<destination>/.sync.lock`, retaken if the holder PID is dead); excludes from the gitignored `.kb-ignore` chain (globals `./root/.kb-ignore` + per-drive `./root/gdrive/<drive name>/.kb-ignore`, e.g. `*.tmp`); writes `./root/gdrive/.sync-reports/sync-<iso>.report` (0600) with remote/local/excluded/dups table + COPY/UPDATE/DELETE + Files excluded + Duplicates ignored + not-downloaded sections. Run `make kb-index KB=gdrive` after to reconcile the synced tree into the `gdrive` KB. The sync and index stages are split so each is independently retryable |
| `kb-index` | POST `/index` alone (no rclone): reconcile `./root/<KB>` trees into their KBs via api-gateway (admin; incremental). `KB=<name>` selects one `./root` subdir + resolves-or-CREATES the KB by name (`scripts/kb-bootstrap.sh` find-or-create: 0 matches → create + grant public read; 1 → reuse + re-grant; >1 → fail — never auto-create a duplicate); no `KB=` indexes EVERY top-level non-dot subdir; `KB=<glob>` (e.g. `xgen-*`) expands against `./root/` (Makefile glob — the gateway never sees a wildcard). `dir=<name>` roots the walk at that single top-dir subdir. `INDEX_ALL=1` for a full re-index. `RETRY_PENDING=1` also re-triggers stalled `pending` files (default retries only `failed`). Per-KB client-side in-flight guard (refuses if `pending+processing>0`; exempt `RETRY_PENDING=1`); fail-fast on `ok=false` |
| `kb-bootstrap` | create a KB named `<name>`, grant public read (`user:*`) (run after `make api-keys`; idempotent; no sidecar). `KB=<name>`; no `KB=` bootstraps every `./root` subdir; with `--resolve`, paginated name→id lookup (unique-or-fail: 0 or >1 matches error). Replaces the old `gdrive-index-bootstrap` (which wrote `GDRIVE_KB_ID` to `.env.local`) |
...
| `kb-desc-backfill` | one-time backfill of the source-attribute kv into existing KB `description` strings (admin). Idempotent + non-destructive: SKIPs any KB whose description already carries a `source=` kv; appends the kv to the prose lead for legacy root (`source=root`) + projects (`source=projects-memory`) KBs; skips unparseable descriptions. `access_grants` is NOT clobbered (`/knowledge/{id}/update` uses `exclude={'access_grants'}`, so the `user:*` public-read grant survives). Each write re-embeds the KB metadata vector (one Ollama call per KB; a failed embed does NOT fail the update). Run on the stack host |
| `kb-status` | GET `/status` (api-gateway) for `KB=<name>`, pretty JSON (indent=2): `indexed_files` (completed only; `len == indexed_count`), `pending_files`, `failed_files` (last list), then `source_count` vs `indexed_count` (completed), `pending` (extraction/OCR) + `processing` (embed+link) + `failed`. Drain terminal when `pending+processing=0` AND `completed+failed>=source_count` |
| `kb-finalize` | finalize a drain: `REINDEX INDEX idx_document_chunk_vector` (pgvector ivfflat) so freshly-embedded vectors become queryable. Polls EVERY `./root/` subdir to a global terminal state first (`pending+processing=0` for all KBs — REINDEX is instance-wide on the shared `document_chunk` table), guarded by a `flock` lock; fails loud if any KB is non-terminal. Run AFTER the drain is terminal (or use `kb-index-finalize` to dispatch + wait). Logs the REINDEX duration. pgvector is the only supported backend; it fails loud on any other `VECTOR_DB` (HNSW-style indexes are incremental, but ivfflat needs `REINDEX`). The BM25 FTS index (`idx_document_chunk_bm25`, ParadeDB `pg_search`) is incremental — no `REINDEX` needed; the old GIN FTS index is gone. Named "finalize" not "reindex": the fresh vectors were never queryable (ivfflat folds post-build rows in only on `REINDEX`; OWUI hybrid = vector-fetch→BM25, no FTS fallback → vector=0 → hybrid=0), and to avoid collision with the gateway `reindex_all` (which re-PROCESSES files) |
| `kb-bm25-init` | create the ParadeDB `pg_search` extension + the `USING bm25` index `idx_document_chunk_bm25` (`text::pdb.simple` tokenizer, `collection_name::pdb.literal` fast field, `key_field='id'`) over the existing `document_chunk` rows, and drop the dead GIN FTS index. Idempotent; container-targeted (`POSTGRES_CONTAINER` or `kb-postgres`). Run once after the kb-postgres image first starts (wired into `make provision` + e2e). No re-embed — the index builds over existing chunks in seconds and is queryable immediately. Fails loud if the `document_chunk` table is absent (run after the stack is up) |
| `kb-bm25-check` | release gate for the BM25 FTS arm: runs `scripts/kb_check.py --bm25-gate` (extension + index + ranking-path `pdb.score` + colon-safe `|||` + zero-token probes). The only detector for the silent-zero / silent-langchain-fallback class (`hybrid_search` swallows paradedb errors → HTTP 200 + 0). A red gate = do not ship |
| `kb-bm25-rollback` | drop the `idx_document_chunk_bm25` index then the `pg_search` extension (order load-bearing — the extension refuses to drop while the index depends on it). Does NOT drop `vector`. Refuses if `pg_search` is already absent. Then revert the OWUI + kb-postgres images |
| `kb-check` | cross-DB health check: reconcile OWUI SQLite + the pgvector `document_chunk` store, report 12 inconsistency classes (1 ghost rows, 2 stale directory_id, 3 orphan file-{id} collections, 4 missing file collections, 5 orphan KB vectors, 6 missing KB vectors, 7 orphan junction rows, 8 dead-KB junction rows, 9 non-completed leftovers, 10 idempotency duplicates, 11 stale root KBs, 12 file rows no knowledge_id), advise the safe repair. `PURGE=1` exports a timestamped backup then purges safe classes (1, 3) via OWUI REST; `PURGE=1 MAINT=1` stops OWUI to also purge maint classes (5b, 7, 8); `REPAIR=1` stops OWUI to repair class-9 stuck-processing-while-linked → completed. `PRUNE_KB=1` deletes stale `source=root` KBs whose `./root/<name>/` dir is gone (class 11): always exports a strict timestamped backup first (mandatory; count-verified), per-KB in-flight guard (refuses if a drain is running), needs OWUI running (incompatible with `MAINT=1`/`REPAIR=1`), separate from `PURGE` (orphan-vector cleanup). `KB=<id>` scopes the KB-tagged classes; `JSON=1` machine-readable; `SHOW_NAMES=1` filenames (default ids-only) |
...
Two pytest fixtures in `tests/conftest.py` own the isolation + provision +
teardown; the test body owns only the test logic:

- **`iso_env`** (session scope) — ONE clean-prod stack shared by the 9 `iso`+
  `shared` tests (`test_04/05/06/07/10/11/13/14/15`). Provisioned once
  (`e2e_provision`); each body self-cleans so the shared stack stays clean.
- **`iso_env_named`** (function scope, a factory) — a named own-iso stack per
  destructive / heavy test: `test_08_e2e` (suffix `test08`), `test_09_gdrive_index`
  (suffix `gdrive`, `at_scale=True`), `test_12_kb_check` (suffix `kbcheck`). Pass
  `at_scale=True` for the comprehensive at-scale provision
  (`e2e_provision_at_scale`: clean-all + image rebuild + preflight + real rclone
  gdrive corpus + `make ci`); otherwise `e2e_provision` (clean-prod).

Select them with `make test-iso` (shared + single short tests) or `make test-long`
(`iso long` = `test_08` + `test_09`). `test_09` is the comprehensive at-scale e2e
(clean-all + image rebuild + rclone + the in-clone suite + the full real-gdrive
drain); `test_12` does the same isolation for the `make kb-check` cross-DB tool
(synthetic fixtures only — no gdrive, no PII). See [testing.md](testing.md) for the
marker → target table + the per-test matrix.

The isolation (clone + compose project + container-rename override +
`OLLAMA_HOST` resolve + `make bootstrap` + teardown) is the **reusable
`scripts/lib-e2e-env.sh` library**, sourced by the conftest iso fixtures
(`tests/conftest.py`) so the isolation logic is not duplicated. `e2e_isolate`
**generates** the container-rename override from `compose.yml`'s service list
(every `container_name` → `kb-<NAME>-<stamp>-*`), merged via `COMPOSE_FILE` — so a
new service added to `compose.yml` is covered automatically (no per-name override
file to keep in sync). The live stack does NOT set `COMPOSE_FILE`, so it keeps the
`kb-*` names.

### Provisioning the clone (the standard way)

The iso env needs two values that differ from the live stack: `KB_HOST`
(`http://localhost:<auto-picked port>` — the iso Caddy port, auto-picked by
`_e2e_free_port` in `3011..3099` skipping `3000`; `KB_HOST_PORT` is derived from
it by `make bootstrap`, not passed) and `OLLAMA_HOST` (the shared external
...
| 5. provision the clone | `e2e_provision` (clean-prod: start + wait healthy + admin-signup + api-keys + ephemeral user + projects-bootstrap + config-rag) OR `e2e_provision_at_scale` (destructive: clean-all + re-bootstrap + restore-creds + [ollama pull OCR model] + preflight + image rebuild + start + ... + kb-bootstrap KB=gdrive + kb-sync + kb-index KB=gdrive + make ci) | `e2e_provision` for the shared/single iso tests; `e2e_provision_at_scale` for `test_09` (`at_scale=True`) |
...
The clone's `.env.template` is **never mutated**. The iso env provisions exactly
the way a live `make bootstrap KB_HOST=... OLLAMA_HOST=...` would (with
`KB_HOST_PORT` derived from `KB_HOST`).

### Proliferation + cleanup (`make clean-test`, `make clean-tests`)
...
container-name prefix: live services like `kb-api-gateway` / `kb-markitdown-ocr`
contain hyphens, so a `kb-<name>-*` name prefix would match the live stack. The
live compose project is the dir basename (`knowledgebase`), never `kb-*`, so
label sweeps never touch the live stack.

### Why `unset BASH_ENV`

`~/.bash_env` is sourced by `BASH_ENV` in every non-interactive bash child (make
recipe shells, `bootstrap.sh`, the iso provision, `admin-signup.sh`, ...) and
...
already exists there). `unset BASH_ENV` (in `e2e_isolate`) stops the re-source;
`OLLAMA_HOST` reaches children via the wrapper's export + normal inheritance, and
`KB_HOST` via `.env` (sourced after any residual profile in every script, so
`.env` wins regardless). This is isolation from the operator's shell profile,
not a workaround. (`unset KB_HOST` drops the live-stack value the wrapper's own
...
a failing run both are left for debugging (`e2e_stop_docker`; flush with
`make clean-test NAME=<suffix>` — `gdrive` for `test_09`, `kbcheck` for `test_12`
— or `make clean-tests` to flush all).

## Repointing OLLAMA_HOST

When the Ollama host moves (Ollama relocated, or switching from a remote GPU host to host-local, or to `localhost`), two services hold the URL and must be re-pointed. `localhost`/`127.0.0.1` is allowed — the graphiti + markitdown-ocr containers translate it to `host.docker.internal` at start, and `make config-rag` writes the translated value to the Open WebUI DB.

This is **not** a re-provision: no `make bootstrap`, no `make pull-models`, no `make api-keys`.

1. Set `OLLAMA_HOST` in your shell env or `.env` (`http://localhost:11434` if host-local; `http://<host>:11434` for a remote/LAN GPU host).
2. `make preflight` — probes the host Ollama at the new URL (host-side, untranslated) and warns if the OWUI DB `rag.ollama.base_url` has drifted.
3. `make restart` — recreates `graphiti` + `markitdown-ocr`; they re-read `OLLAMA_HOST` and the entrypoint shim translates `localhost`→`host.docker.internal`. (OWUI restarts too but reads its DB, not the env.)
4. `make config-rag` — re-syncs OWUI's `rag.ollama.base_url` DB value to the new, translated `OLLAMA_HOST`. **Mandatory for OWUI**: it persists the URL and ignores the env after first boot. (For the OWUI **chat** URL — a separate persisted key — use the admin API `POST /ollama/config/update`; the RAG sync does not change it.)
5. `make health` — confirm.
...
- `.env` is gitignored (copy from `.env.template`) and holds no secrets (ports, tags, model names, tunables, `KB_HOST_PORT`, `OCR_ENABLED`, `HOST_UID`/`HOST_GID` (api-gateway run user, auto-set by `make bootstrap`), `NEO4J_URI`) — except `OLLAMA_HOST` and `KB_HOST`, which are deployment-specific (commented in the template; set in the shell env so they override).
- `.env.local` is gitignored (`chmod 0600`) and holds:
  - `WEBUI_SECRET_KEY` (generated by `make bootstrap`).
  - `OPENWEBUI_ADMIN_API_KEY` (generated by `make api-keys`) — the admin key, a valid `KB_API_KEY`.
  - Operator user keys (`KB_API_KEY`, non-admin) come from `make users-create` and live in `~/.api_keys`, not `.env.local`.
  - `OCR_SERVICE_TOKEN` (generated by `make bootstrap` when `OCR_ENABLED=true`; the bearer OWUI sends to `markitdown-ocr`; absent when `OCR_ENABLED=false`).
...
- If `KB_HOST` is reachable from an untrusted network:
  ...
  - Limit the port (`KB_HOST_PORT`, derived from `KB_HOST`) with a firewall to trusted LAN/VPN.
