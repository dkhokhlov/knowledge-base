...
- `OLLAMA_HOST` — Ollama's native client env var: the full URL where [Graphiti][graphiti] and [Open WebUI][open-webui] reach [Ollama][ollama]. Set in shell env or `.env` (shell overrides `.env`). `localhost`/`127.0.0.1` is allowed (the shell convention for a host-local Ollama): the graphiti + markitdown-ocr containers run a small entrypoint shim (`scripts/ollama-host.sh`) that rewrites it to `host.docker.internal` before the app reads it (compose adds `extra_hosts: host-gateway`). A non-localhost URL (a remote/LAN GPU host) passes through unchanged. See [Repointing OLLAMA_HOST](#repointing-ollama_host) for the move procedure.
  ...
  - [Open WebUI][open-webui] persists the Ollama URL in its DB on **first boot** and ignores later env changes. It does **not** use the shim; `make rag-config` (part of `make provision`) writes the translated value (`host.docker.internal`) to the DB.
    - Chat URL: admin API `POST /ollama/config/update` with `{"ENABLE_OLLAMA_API":true,"OLLAMA_BASE_URLS":["http://<ollama-host>:11434"],"OLLAMA_API_CONFIGS":{}}`, or wipe `./data/openwebui` and restart. Use the **container-reachable** value (`http://host.docker.internal:11434` if Ollama is host-local), not `localhost` — OWUI chat does **not** use the entrypoint shim, so a `localhost` chat URL points at the container's own loopback (no Ollama). `make rag-config` does not touch this key either.
    - RAG embedding URL (`rag.ollama.base_url`) is a **separate** persisted key; the chat update above does **not** change it. A stale value leaves chat working but breaks file embedding (`process/status` = `failed`, RAG search returns 0 hits). `make rag-config` syncs it to the translated `OLLAMA_HOST` (`host.docker.internal`, not `localhost` — the same translation the shim applies; idempotent); `make preflight` warns if it has drifted. `make test` (test_04) catches this drift.
...
| `OLLAMA_HOST` | Ollama URL (full; chat LLM + embeddings); shell env or `.env`. `localhost`/`127.0.0.1` allowed (translated to `host.docker.internal` in-container for graphiti/markitdown-ocr; `make rag-config` translates for the OWUI embedding DB) |
...
Then run `make kb-bootstrap KB=gdrive` (re-assert the grant) + `make kb-sync` +
`make kb-index KB=gdrive` (incremental — `dir=gdrive` keeps manifest keys identical
to the old `source=gdrive`, so no mass drain).
...
| RAG search returns 0 hits; file embedding `process/status = failed` | `rag.ollama.base_url` stale (OWUI persists it on first boot, ignores later `OLLAMA_HOST` changes) | `make rag-config` (re-syncs to the translated `OLLAMA_HOST`); `make preflight` warns on drift; test_04 catches it |
...
| `provision` | ONE-TIME from-scratch setup: `bootstrap` → `pull-models` (blocking) → `start` → `admin-signup` → `api-keys` (auto OCR config) → `rag-config` → `kb-bm25-init` → `kb-bootstrap KB=gdrive`; leaves the stack running. Re-run after `make clean-all`; for everyday restart use `make start` |
...
4. `make rag-config` — re-syncs OWUI's `rag.ollama.base_url` DB value to the new, translated `OLLAMA_HOST`. **Mandatory for OWUI**: it persists the URL and ignores the env after first boot. (For the OWUI **chat** URL — a separate persisted key — use the admin API `POST /ollama/config/update`; the RAG sync does not change it.)
