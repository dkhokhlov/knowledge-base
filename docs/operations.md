...
- `.env` — gitignored, copied from `.env.example` (the tracked template). No secrets; set `OLLAMA_HOST` to your Ollama URL here:
  - ports, image tags, model names, Neo4j memory, tunables, `OLLAMA_HOST`, `MARKITDOWN_OCR_PROVISIONED` (OCR-provisioned marker, set by `make ocr-bootstrap`).
...
- `.env.local` — gitignored, `chmod 0600`. Holds:
  ...
  - `OCR_SERVICE_TOKEN` (generated by `make ocr-bootstrap`; the bearer OWUI sends to `markitdown-ocr`).
...
| `ocr-bootstrap` | build + start `markitdown-ocr`, point OWUI at it (`CONTENT_EXTRACTION_ENGINE=external`), write `MARKITDOWN_OCR_PROVISIONED=1` to `.env` (run after `make api-keys`; idempotent; no fallback) |
...
| `clean-all` | `down --volumes` + delete `./data` + delete `./.gdrive-backup/` + delete `.env.local` + drop the `MARKITDOWN_OCR_PROVISIONED` marker from `.env` |
...
- `clean-all` keeps `.env` config (drops the `MARKITDOWN_OCR_PROVISIONED` marker), `graphiti/config.yaml`, and `caddy/Caddyfile`.
...
- `.env` is gitignored (copy from `.env.example`) and holds no secrets (ports, tags, model names, tunables, `KB_HOST`, `KB_HOST_PORT`, `MARKITDOWN_OCR_PROVISIONED`) — except `OLLAMA_HOST`, which is deployment-specific.
