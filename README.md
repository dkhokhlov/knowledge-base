...
- `make gdrive-status` reads GET `/status`: `source_count` (allowlisted `gdrive/` file count) vs `indexed_count` (files with `data.status=completed` — extracted, embedded, linked, searchable), `pending` (in extraction / OCR / GPU, or queued), `processing` (embedding + linking), `failed` (`{filename, error}`), and a `✓ COMPLETE` / `○ in-flight=N` status line. The drain is terminal when `pending+processing=0` AND `completed+failed` covers `source_count`. No ETA (no daemon).
...
- Subpath reconcile: `/index` and `/status` accept `?path=<relpath>` (relative to the gdrive root; a directory or a single file) — exposed as `make gdrive-sync --path <relpath>` / `make gdrive-index PATH=<relpath>` / `make gdrive-status PATH=<relpath>`. `path` is a SOURCE FILTER only: it scopes the walk to that subpath (entry keys stay root-relative, so a later full `/index` sees `unmodified`). The reconcile is a FULL reconcile of that subpath — files removed from the source under it ARE removed from the KB — so use a KB whose whole scope is that `path` (a dedicated/subpath KB; on a shared KB `path` deletes every KB file outside the subpath). `/status?path=` scopes `source_count` to the subpath (file counts are KB-wide). The full walk skips dot-dirs/dot-files (hidden metadata like `.sync-reports`/`.sync.lock` is never indexed); `path` opts into a dot-subtree.
