...
**Add another KB** (any local folder tree): drop/symlink/bind-mount it at `./root/<name>/`, then `make kb-bootstrap KB=<name>` + `make kb-index KB=<name>`. Each top-level subdir of `./root/` is one KB, named after the subdir. Migrate an existing `./gdrive` deployment with `make kb-migrate-root` (one-time).
...
- **`make kb-index KB=<name>`** runs POST `/index?dir=<name>` alone (no rclone): reconcile `./root/<name>/` into its KB. `KB=<name>` selects one `./root` subdir; no `KB=` indexes EVERY top-level non-dot subdir; `KB=<glob>` (e.g. `xgen-*`) expands against `./root/` (Makefile glob — the gateway never sees a wildcard). Default **incremental**; set **`INDEX_ALL=1`** for a full re-index (drain + re-upload every file). Set **`RETRY_PENDING=1`** to also re-trigger stalled `pending` files (delete + re-upload; the default retries only `failed`). Per-KB client-side in-flight guard + name resolution; the `.tests` dot-dir is indexed only by its own test.
