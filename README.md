...
2. Copy the printed `GRAPHITI_API_TOKEN` into your MCP clients (sent as `Authorization: Bearer <token>`).
   - `WEBUI_SECRET_KEY` is internal to Open WebUI; no action needed.
...
- `.env.local` — gitignored, `chmod 0600`. Holds the two secrets:
  ...
  - `GRAPHITI_API_TOKEN` (generated by `make bootstrap`, printed to the terminal; copy into MCP clients).
