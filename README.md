...
- **`make gdrive-sync`** syncs `./root/gdrive` from all shared drives (rclone `sync --backup-dir --delete-after`; delta — files removed from Drive are deleted from `./root/gdrive`, and the next `/index` drops them from the KB via `sync/cleanup`). Deleted/overwritten files are retained in a dated `./.gdrive-backup/<UTC-ISO>/` dir outside `./root` as a recovery net; `make clean-backup` clears it.
- Exclude rules: per-drive non-downloadable files and global patterns (e.g. `*.tmp`) are read from the gitignored `./root/.exclude.conf` (INI format: `[*]` global + `[gdrive/<drive name>]` per-drive sections; format documented in the tracked `./root/.exclude.conf.example`) and converted to `--exclude-from` per drive. `[*]` is a global deny-list: its patterns are not indexed in ANY KB.
