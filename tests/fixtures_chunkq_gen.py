...
html, json, log, tex) into --out (default root_tests/chunkq) and prints a
...
The generated files are COMMITTED (tracked in git) at root_tests/chunkq/
alongside the test_11 fixture set. Regenerate them with:
  python3 tests/fixtures_chunkq_gen.py --out root_tests/chunkq
...
SKELETON_DIR = REPO / "root_tests" / "gdrive"
...
def _docx_bytes(sec):
    """Clone root_tests/gdrive/fixture-doc.docx; swap in Heading1/Heading2 paragraphs.
...
def _pptx_bytes():
    """Clone root_tests/gdrive/fixture-slide.pptx and grow it to 4 slides.
...
def main(argv=None):
    ...
    parser.add_argument(
        "--out", default="root_tests/chunkq", help="output directory (default: root_tests/chunkq)"
    )
    parser.add_argument(
        "--types",
