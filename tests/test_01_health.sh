...
# and the api-gateway auth gate (via Caddy). No auth required.
set -u
. "$(dirname "$0")/lib.sh"
load_env
require_stack_up

# KB_HOST is the single public URL: Caddy -> api-gateway (/memory/*, /health) and
...
# KB_HOST (:3000) is Caddy -> api-gateway /health (reflects the OWUI identity dep).
code=$(http_code "$H/health")
[ "$code" = 200 ] && pass "api-gateway /health (via Caddy) -> 200" || fail "api-gateway /health -> $code (want 200)"
